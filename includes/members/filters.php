...
function appp_add_data_to_activity_api_items( $response, $request, $activity ) {
	...
	// Allow extra HTML to be added after activity content.
	// This is here in case plugins add their own content to activity that is not part of BPRE api.
	$response->data['html'] = apply_filters( 'bp_rest_activity_html', '', $response->data );
	...
	if ( $logged_in ) {
		...
		if ( isset( $activity->user_id ) && ( get_current_user_id() === $activity->user_id ) ) {
			$can_delete = true;
		}
	...
	$response->data['can_delete'] = $can_delete;
...
*/
	switch ( $response->data['type'] ) {
		...
		case 'activity_comment':
			if ( 'activity' === $activity->component ) {
				...
				$response->data['attachments'] = apply_filters( 'bp_rest_activity_comment_attachments', $attachments, $activity );
			}
			break;
		...
		case 'activity_update':
			...
			if ( 'activity' === $activity->component ) {
				...
				$response->data['attachments'] = apply_filters( 'bp_rest_activity_update_attachments', $attachments, $activity );
			...
			if ( 'groups' === $activity->component ) {
				...
				$response->data['activity_action'] = array( 'action' => 'Posted an update in ' . $group->name );

				$attachments = appp_get_activity_attachments( $activity->id );

				$response->data['attachments'] = apply_filters( 'bp_rest_activity_group_attachments', $attachments, $activity );
	...
	return apply_filters( 'appp_rest_activity_prepare_value', $response, $activity );
}
 ...
 * @return WP_REST_Response
 */
function appp_add_data_to_members_api_items( $response, $request, $member ) {
